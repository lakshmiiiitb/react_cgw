...
export function Delivery(){
    ...
    const submitresponse= (event) => {
        ...
        axios.post(base_url+"/register/delivery",credentials)
            .then(response => {
    ...
    return (
        <>
            ...
            <div >
                <div><h1>DELIVERY REGISTRATION FORM</h1><br/><br/>
                </div><br/>
            <form style={{textAlign:'center'}} onSubmit={submitresponse}>
                <div>
                    <label>Name:</label><span style={{ marginLeft: "30px" }}>
                    <input type="text"  onChange={(e)=>{setCredentials({...credentials,name:e.target.value})}}/>
                </span>
                </div><br/><br/>
                <div><label>Phone:</label><span style={{ marginLeft: "30px" }}>
